...
class Square:
    ...
    def build(self):
        ...
        for i in range(num):
            ...
            if i % (num//10) == num//10-1:
                print("*\n" if i == num-1 else ".", end='')
        self.k_to_Tc = interpolate.interp1d(table_k, table_Tc, fill_value=(
            table_Tc[0], table_Tc[-1]), bounds_error=False)
    ...
    def Tc_binary(self, k):
        Js = self.gen_Js(k)
        def f(T): return np.prod(np.sinh(self.z(Js, T)))-1
...
class Triangle:
    ...
    def Tc(self, k1, k2):
        ...
        def f(T): return np.dot(self.z(Js, T), np.roll(self.z(Js, T), 1))-1
...
class Cubic:
    sample = 10
    ratio = np.array([1.0, 0.5, 0.2, 0.1, 0.05, 0.02,
                      0.01, 0.005, 0.002, 0.001])
    Tc_coeff = np.array([4.5117, 2.9297, 1.8139, 1.34307,
                         1.03984, 0.78297, 0.65251, 0.55589, 0.46200, 0.40832])
